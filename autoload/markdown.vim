...
"    let figlet = split(system('pyfiglet -w '.&columns.' -f '.font.' '.shellescape(substitute(a:text,'^#\+s*','',''))), "\n")

  " Headings - Centered Normal Text for ####
  elseif a:text =~? '^####[^#]'
    let new_text += s:Center([substitute(a:text,'^####\+s*','','')], '')

  elseif a:text =~? '^###[^#]' && g:presenting_figlets
    let inttext = substitute(a:text,'^###\+s*','','')
    let figlet = split(system('pyfiglet -w 127 -f digital '.shellescape(inttext)), "\n")
    let new_text += s:Center(figlet, '')
    
  elseif a:text =~? '^##[^#]' && g:presenting_figlets
    let inttext = substitute(a:text,'^##\+s*','','')
    let figlet = split(system('pyfiglet -w 127 -f chunky '.shellescape(inttext)), "\n")
    let new_text += s:Center(figlet, '«h2»')

  " Headings - Centered Figlets for #, ##, and ###
  elseif a:text =~? '^#[^#]' && g:presenting_figlets
    let inttext = substitute(a:text,'^#\+s*','','')
    let figlet = split(system('pyfiglet -w 127 -f doom '.shellescape(inttext)), "\n")
    let new_text += s:Center(figlet, '«h1»')
